...
namespace Interop.SocketLib {
  ...
  public class RawScktServer : RawSocket {
    ...
    private async void ServerListener() {
      string strAnswer;
      try {
        strAnswer = ndStatus.Invoke(await ParseReceived(), hc4x_SocketStatus.Received);
        await SendMessage(strAnswer);
      }
